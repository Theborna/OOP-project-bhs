...
public class UserDB {
    ...
    public static ArrayList<User> searchByUserName(String entry, long userid) throws SQLException {
        Connection con = getConnection();
        Statement st = con.createStatement();
    ...
    public static void Unblock(long currentUser, long touBlock) throws SQLException {
        Connection con = getConnection();
        String query = "delete from block where user_id = " + currentUser + " and blocked_id = " + touBlock;// TODO: add pfp
        // System.out.println(query);
        con.createStatement().execute(query);
        con.close();
    }
