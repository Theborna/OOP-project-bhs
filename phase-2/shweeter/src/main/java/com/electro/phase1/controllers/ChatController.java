...
public class ChatController {
    ...
    // case "down":
    // case "d":
    // currentMsg = messages.get((++current < messages.size()) ? current : (current
    // = 0));
    // break;
    // case "l":
    // case "last":
    ...
    // public StringBuilder getMsgText(StringBuilder msgText) throws
    // changeViewException {
    // if (msgText == null)
    ...
    public static void post(StringBuilder sb, Message message2) {
        if (sb == null)
            return;
        Message message = new Message(sb.toString(), User.getCurrentUser());
        message.setReplyTo(message2);
