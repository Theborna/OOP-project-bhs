...
public class UserFollowingConnection extends connection<User,User>{
    ...
    public static Set<UserFollowingConnection> getFollowings(User user) {
        ...
        for (int i = 0; i < 10; i++) {
            result.add(new UserFollowingConnection(user,new NormalUser(Integer.toBinaryString(i), Integer.toString(2*i))));
        }
        return result;
    }
    ...
    public static Set<UserFollowingConnection> getFollowers(User user) {
        ...
        for (int i = 0; i < 10; i++) {
            result.add(new UserFollowingConnection(new NormalUser(Integer.toBinaryString(i), Integer.toString(2*i)), user));
        }
        return result;
    }
